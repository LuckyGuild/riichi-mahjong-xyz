...
// Given m: Meld, we want to produce a single { type: 'kotsu'|'shuntsu'; tile: number } for each meld
function meldToBlock(m: Meld): Block {
  switch (m.type) {
    case 'chii': {
      // A chii has 3 consecutive tiles in the same suit
      // The block type is 'shuntsu'
      // We'll pick the *lowest* tile to represent it
      // Typically you'd confirm they are indeed consecutive,
      // but presumably that was done when forming the meld.
      const sorted = [...m.tiles].sort(compareTiles); // ascending
      const lowest = sorted[0];
      return {
        type: 'shuntsu',
        tile: tileToCountsIndex(lowest),
      };
    }
    case 'pon': {
      // 3 identical tiles => block type 'kotsu'
      // representative tile = m.tiles[0]
      const base = m.tiles[0];
      return {
        type: 'kotsu',
        tile: tileToCountsIndex(base),
      };
    }
    case 'kan': {
      // 4 identical tiles => also 'kotsu'
      // (some logic might treat kan differently if closed vs open,
      //  but block-based code typically just calls it a kotsu.)
      const base = m.tiles[0];
      return {
        type: 'kotsu',
        tile: tileToCountsIndex(base),
...
export const generateHora = (
  ...
  // Flatten actual tiles from melds:
  const meldTiles = input.melds.flatMap(m => m.tiles); // removed instantiateMeld

  // Build one big list of the player's entire set of tiles:
    ...
    (() => {
      ...
      (() => {
        ...
        const handAndMeldBlocks = [
          ...
          ...input.melds.map(m => meldToBlock(m)) // revised
        ];
        const allTiles = [...combinedHand, horaTile, ...meldTiles];
        const allTilesCounts = tilesToCounts(allTiles);

        const allMentsu = [
          ...decomposeResult.blocks.filter(b => b.type === 'kotsu' || b.type === 'shuntsu'),
          ...input.melds.flatMap(m => [meldToBlock(m)]),
        ];
        ...
        // Is not a “honor” t.type !== 'z'
        // Has the number 5 t.n === 5
        // Has the property red === true
        const r = countBy(allTiles, t => {
          if (t.type === 'z') return false;
          if (t.n !== 5) return false;
          return t.red === true;
        });
        if (r > 0) {
          yaku.push({
...
export const generateChitoitsuHora = (
  ...
  return horaTiles.flatMap(horaTile => {
    ...
    const r = countBy(all, t =>
      t.type !== 'z' && t.n === 5 && t.red === true
    );
    if (r > 0) {
      yaku.push({
        yaku: { type: 'yaku', name: 'red-dora', han: r },
