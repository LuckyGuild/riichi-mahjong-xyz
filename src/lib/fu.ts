...
export const fu = (
  ...
  ret.push(
    ...melds.flatMap<Fu>((m) => {
      if (m.type === 'chii') {
        const sorted = [...m.tiles].sort(/* your compare logic */);
        const tileIndex = tileToCountsIndex(sorted[0]);
        const fuObj: ShuntsuFu = {
          type: 'shuntsu',
          tile: tileIndex,
          fu: 0
        };
        return [fuObj]; // single-element array
      }
      else if (m.type === 'pon') {
        const base = m.tiles[0];
        const tileIndex = tileToCountsIndex(base);
        const isHonorOrTerminal = (base.type === 'z' || base.n === 1 || base.n === 9);
        const fuValue = isHonorOrTerminal ? 4 : 2;
        const fuObj: KotsuFu = {
          type: 'kotsu',
          tile: tileIndex,
          kotsuType: 'minko',
        ...
        const base = m.tiles[0];
        const tileIndex = tileToCountsIndex(base);
        const kotsuType = m.closed ? 'ankan' : 'minkan';
        const isHonorOrTerminal = (base.type === 'z' || base.n === 1 || base.n === 9);
        let fuValue: 8 | 16 | 32 = 8;
        if (m.closed) {
          fuValue = isHonorOrTerminal ? 32 : 16;
        } else {
          fuValue = isHonorOrTerminal ? 16 : 8;
        }
        const fuObj: KotsuFu = {
          type: 'kotsu',
          tile: tileIndex,
