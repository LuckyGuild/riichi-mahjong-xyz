...
export const Segment: FC<SegmentProps> = ({
  ...
  <div className="flex select-none divide-x divide-neutral-300 overflow-hidden rounded-md border border-neutral-300 shadow transition dark:divide-neutral-700 dark:border-neutral-700">
    {items.map((item, i) => (
      <button
        key={i}
        className="flex-1 whitespace-nowrap bg-white p-1 transition hover:bg-neutral-200 disabled:bg-[#EB0A1E] disabled:text-white dark:bg-black dark:hover:bg-neutral-800 dark:disabled:bg-[#EB0A1E] dark:disabled:text-white"
        disabled={index === i}
        onClick={() => onChange(i)}
      >
        {item}
